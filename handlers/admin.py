...
Не прошли обучение: {a[1]}
Обучились, но не прошли прогрев: {a[2]}
Прошли обучение и прогрев: {a[3]}

Неопознанный трафик: {a[4]}
""")
...
async def check(call: types.callback_query, state: FSMContext):
    ...
    await bot.send_message(call.message.chat.id, 'Ебашим рассылку?', reply_markup = murkap)
    await bot.answer_callback_query(call.id)
