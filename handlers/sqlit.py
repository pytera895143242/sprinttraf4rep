...
def reg_user(id):
    ...
    if sql.fetchone() is None:
        sql.execute(f"INSERT INTO user_time VALUES (?,?)", (id,'null'))
        db.commit()
...
def members_list():
    ...
    null = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'null'" ).fetchone()[0]
    one = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'one'").fetchone()[0]
    two = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'two'").fetchone()[0]

    another = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'wait'").fetchone()[0]


    return [all, null,one,two,another]
